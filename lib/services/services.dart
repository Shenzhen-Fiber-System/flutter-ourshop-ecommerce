
export 'package:ourshop_ecommerce/services/communication/communication_service.dart';
export 'package:ourshop_ecommerce/services/products/product/product_service.dart';
export 'package:ourshop_ecommerce/models/social_medias/social_media.dart';
export 'package:ourshop_ecommerce/services/orders/order_service.dart';
export 'package:ourshop_ecommerce/services/products/category/category_service.dart';
export 'admin/admin_services.dart';
export 'package:ourshop_ecommerce/services/dio.dart';
export './service_locator.dart';
export 'package:ourshop_ecommerce/services/social_media/social_media_service.dart';
export 'package:ourshop_ecommerce/services/stripe_service.dart';